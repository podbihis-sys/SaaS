...
export function generateStaticParams() {
  ...
  for (const t of applicationTaxa()) {
...
export async function generateMetadata({
  ...
  const description = `${r.category.name} für die Anwendung „${r.taxon.label}" von BIT: ${r.products.length} Artikel mit allen verfügbaren Größen direkt anfragbar.`;
  return {
...
export default async function ApplicationCategoryLanding({
  params,
}: {
  params: Promise<{ slug: string; kategorie: string }>;
}) {
  ...
  const intro = `${category.name} für die Anwendung „${taxon.label}". ${taxon.intro}`;
