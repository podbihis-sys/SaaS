...
export function generateStaticParams() {
  return shrinkTaxa().map((t) => ({ slug: t.slug }));
}
...
export async function generateMetadata({
  ...
  if (!taxon) return { title: "Schrumpfrate nicht gefunden" };
  const title = `Schrumpfschlauch ${taxon.label} – Schrumpfrate ${taxon.label}`;
  return {
...
export default async function ShrinkLanding({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <TaxonLanding
      kind="Schrumpfrate"
      label={`Schrumpfrate ${taxon.label}`}
      heading={`Schrumpfschläuche mit Schrumpfrate ${taxon.label}`}
      intro={taxon.intro}
