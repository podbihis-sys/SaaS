...
const PAINS = [
  { accent: "from-rose-500 to-red-500", title: "Das Excel-Dilemma", text: "Ihre Prüfliste lebt in einer Tabelle, die nur eine Person kennt. Ist die im Urlaub, weiß niemand, welche Geräte fällig sind — gemerkt wird es erst, wenn der Prüfdienstleister anruft.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M3 3h18v18H3zM3 9h18M9 9v12M15 9v12" /> },
  { accent: "from-amber-500 to-orange-500", title: "Der Moment bei der BG-Kontrolle", text: "Der Prüfer will die Protokolle der letzten zwei Jahre sehen — für alle Geräte, Leitern, Feuerlöscher. Sie suchen in Mails und Ordnern. Notiert wird: „unvollständige Dokumentation“.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7l5 5v11a2 2 0 0 1-2 2z" /> },
  { accent: "from-slate-600 to-slate-800", title: "Persönliche Haftung", text: "ArbSchG §13, DGUV Vorschrift 1, BetrSichV — die Nachweispflicht liegt bei Ihnen persönlich. Bei einem Unfall mit ungeprüftem Gerät wird genau diese Dokumentation zur entscheidenden Frage.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 3l8 4v5c0 5-3.4 8-8 9-4.6-1-8-4-8-9V7l8-4z" /> },
];

const FEATURES = [
  { accent: "from-blue-500 to-indigo-600", title: "Inventar mit QR-Code-Etiketten", text: "Jedes Prüfobjekt bekommt ein druckbares QR-Etikett — Smartphone draufhalten, Geräteakte offen. Kein Suchen mehr.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h3v3h-3zM20 14v6m0 0h-3" /> },
  { accent: "from-sky-500 to-cyan-500", title: "Automatische Fristen­überwachung", text: "E-Mails 60, 30 und 7 Tage vor jedem Termin — plus Eskalation bei Überfälligkeit. Keine Frist fällt mehr durchs Raster.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.4-1.4A2 2 0 0 1 18 14v-3a6 6 0 1 0-12 0v3a2 2 0 0 1-.6 1.4L4 17h5m6 0v1a3 3 0 1 1-6 0v-1m6 0H9" /> },
  { accent: "from-violet-500 to-purple-600", title: "Lückenlose Dokumentation", text: "Datum, Prüfer, Ergebnis, PDF-Nachweis — jeder Vorgang mit Zeitstempel, nachträglich nicht änderbar, nur ergänzbar.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m1 8H8a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5l5 5v7a2 2 0 0 1-2 2z" /> },
  { accent: "from-emerald-500 to-teal-600", title: "Prüfbericht per Knopfdruck", text: "Ein Klick erzeugt das vollständige PDF mit allen Geräten, Fristen und Nachweisen — genau das Dokument für die BG-Kontrolle.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v12m0 0l-4-4m4 4l4-4M5 21h14" /> },
  { accent: "from-amber-500 to-orange-600", title: "Gesetzliche Vorlagen inklusive", text: "DGUV V3, Leitern, Feuerlöscher, Erste Hilfe, UVV — mit hinterlegten Standard-Intervallen, je Gerät anpassbar.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h10" /> },
  { accent: "from-blue-600 to-slate-700", title: "DSGVO-konform in Deutschland", text: "Alle Daten in Frankfurt am Main, kein Transfer in Drittländer, Auftragsverarbeitungsvertrag inklusive.", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 3l8 4v5c0 5-3.4 8-8 9-4.6-1-8-4-8-9V7l8-4zM9.5 12l1.8 1.8L15 10" /> },
];
...
const FAQS = [
  ...
  { q: "Sichert PrüfPilot meine Haftung ab?", a: "PrüfPilot liefert die Dokumentation, die Sie als Betreiber nach ArbSchG, DGUV Vorschrift 1 und BetrSichV schulden. Diese Nachweiskette ist der entscheidende Faktor, wenn Behörde, BG oder Versicherung die Sorgfaltspflicht bewerten. PrüfPilot ersetzt keine Rechtsberatung — es schafft deren Grundlage." },
    ...
    <div className="overflow-x-hidden bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-blue-50 via-white to-white">
        <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
          <div className="absolute -left-20 -top-24 h-96 w-96 rounded-full bg-blue-300/40 blur-3xl animate-blob" />
          <div className="absolute -right-16 top-0 h-80 w-80 rounded-full bg-indigo-300/40 blur-3xl animate-blob [animation-delay:3s]" />
          <div className="absolute left-1/3 top-24 h-72 w-72 rounded-full bg-cyan-200/40 blur-3xl animate-blob [animation-delay:6s]" />
        </div>
        <div className="relative mx-auto max-w-5xl px-6 pb-16 pt-20 text-center">
          <div className="inline-flex animate-fade-up items-center gap-2 rounded-full border border-blue-100 bg-white px-4 py-1.5 text-sm font-medium text-blue-700 shadow-sm">
          ...
          <h1 className="animate-fade-up mt-6 text-4xl font-extrabold leading-[1.1] tracking-tight text-slate-900 [animation-delay:80ms] sm:text-6xl">
            Prüfpflichten im Griff.
            <br />
            <span className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 bg-[length:200%_auto] bg-clip-text text-transparent animate-gradient">Haftung abgesichert.</span>
          </h1>
          ...
          <div className="animate-fade-up mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-slate-500 [animation-delay:320ms]">
            {TRUST.map((item) => (
              <span key={item} className="inline-flex items-center gap-1.5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-4 w-4 text-blue-600"><path strokeLinecap="round" strokeLinejoin="round" d="M20 6 9 17l-5-5" /></svg>
                {item}
              </span>
            ))}
          </div>
          <div className="animate-fade-up relative z-10 mx-auto mt-16 max-w-3xl [animation-delay:420ms]">
            <div className="absolute -inset-x-8 -top-6 bottom-0 -z-10 rounded-[2.5rem] bg-gradient-to-tr from-blue-300/40 to-cyan-200/40 blur-3xl" aria-hidden="true" />
      ...
      <div className="border-y border-slate-100 bg-white py-8">
        <p className="text-center text-xs font-semibold uppercase tracking-wider text-slate-400">Gemacht für deutsche KMU aus</p>
        <div className="mx-auto mt-5 flex max-w-4xl flex-wrap items-center justify-center gap-x-10 gap-y-3 px-6">
          {INDUSTRIES.map((name) => (
            <span key={name} className="text-base font-semibold text-slate-400">{name}</span>
          ))}
        </div>
      </div>
          ...
          <Reveal delay={120}>
            <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
              {DEVICE_CATEGORIES.map((c) => (
                <span key={c.id} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-600">{c.nameDe}</span>
              ))}
            </div>
          </Reveal>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="mt-12 grid items-center gap-10 md:grid-cols-2">
            ...
            <Reveal delay={120}>
              ...
              <p className="mt-3 text-slate-600">Jedes Gerät trägt sein eigenes Etikett. Vor Ort scannen, Prüfung dokumentieren — der Nachweis landet automatisch in der unveränderlichen Historie.</p>
              <ul className="mt-6 space-y-3 text-sm text-slate-600">
                {SHOWCASE_POINTS.map((point) => (
                  <li key={point} className="flex items-start gap-2.5"><Check />{point}</li>
                ))}
              </ul>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-3xl px-6">
          <Reveal className="text-center"><h2 className="text-3xl font-bold tracking-tight text-slate-900">Häufige Fragen</h2></Reveal>
      ...
      {/* CTA-Panel */}
      <section id="vormerken" className="bg-white px-6 py-20">
        <div className="relative mx-auto max-w-5xl overflow-hidden rounded-[2rem] bg-gradient-to-br from-blue-600 to-indigo-700 px-8 py-12 text-white shadow-2xl shadow-blue-600/30 sm:px-12">
          <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
            <div className="absolute -right-16 -top-16 h-72 w-72 rounded-full bg-cyan-300/20 blur-3xl animate-blob" />
            <div className="absolute -bottom-20 -left-10 h-72 w-72 rounded-full bg-indigo-400/30 blur-3xl animate-blob [animation-delay:4s]" />
          </div>
              ...
              <ul className="mt-6 space-y-2 text-sm text-blue-100/90">
                {["Persönliches Onboarding beim Einrichten", "Mitsprache bei den nächsten Funktionen", "Vorzugskonditionen zum Start"].map((b) => (
                  <li key={b} className="flex items-center gap-2">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-4 w-4 text-cyan-300"><path strokeLinecap="round" strokeLinejoin="round" d="M20 6 9 17l-5-5" /></svg>
                    {b}
                  </li>
                ))}
              </ul>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-6 py-12 sm:grid-cols-2 md:grid-cols-4">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm text-slate-500">
              ...
              <li>Hosting & Server: Frankfurt a. M.</li>
            </ul>
          </div>
        </div>
